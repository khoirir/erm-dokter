...
func TestLoggingMiddleware_AuthenticatedAndAnonymous(t *testing.T) {
	...
	// 4. Test default LoggingMiddleware constructor
...
func TestLoggingMiddleware_WithErrorDetailAndUser(t *testing.T) {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("X-User-ID", "DRHANDI")
		w.Header().Set("X-Error-Detail", "Autentikasi ditolak: Kredensial login tidak cocok untuk username 'DRHANDI'")
	...
	// Verify headers are stripped from client
	...
	expectedMsg := "Autentikasi ditolak: Kredensial login tidak cocok untuk username 'DRHANDI'"
	if parsed["msg"] != expectedMsg {
		t.Errorf("expected msg '%s', got '%v'", expectedMsg, parsed["msg"])
	}
	if parsed["user"] != "DRHANDI" {
		t.Errorf("expected user 'DRHANDI', got '%v'", parsed["user"])
	}
...
func TestLoggingMiddleware_WithSuccessMessage(t *testing.T) {
	...
	// Verify internal header X-Log-Message is NOT leaked to HTTP client
