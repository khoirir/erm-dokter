...
// TestSimpanResep_ConcurrentDoctors menguji banyak dokter menyimpan resep di waktu yang sama (paralel)
// untuk memastikan tidak terjadi bentrokan nomor resep (race condition).
func TestSimpanResep_ConcurrentDoctors(t *testing.T) {
	...
	mockRepo := &mockConcurrentRepo{
		simpanResepFunc: func(ctx context.Context, kodeDokter string, statusLanjut shared.StatusLanjut, req SimpanResepRequest) (*Resep, error) {
			mu.Lock()
			// Simulasi generate nomor resep unik urut dengan locking
	...
	for i := 1; i <= totalDokter; i++ {
		...
		go func(dokterIdx int) {
			defer wg.Done()
			<-startGate // Tunggu sinyal mulai serentak
			...
			// Simpan ke map untuk mengecek apakah ada duplikasi no_resep
	...
	// Tembak 20 request dokter secara bersamaan
	close(startGate)
	wg.Wait()
	...
	// Hitung total nomor resep unik yang berhasil digenerate
	totalUnik := 0
...
// TestIsDuplicateKey menguji deteksi error MySQL Duplicate Primary Key (Error 1062)
