...
func (r *repository) RiwayatPenilaianMedisRalanByNoRM(ctx context.Context, noRM string) ([]PenilaianMedisRalan, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("gagal scan riwayat penilaian medis ralan: %w", err)
		}
...
func (r *repository) CekPenilaianMedisRalanAda(ctx context.Context, noRawat string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("gagal cek penilaian medis ralan: %w", err)
	}
	return count > 0, nil
}
...
func (r *repository) HapusPenilaianMedisRalan(ctx context.Context, noRawat string) error {
	...
	if err != nil {
		return fmt.Errorf("gagal hapus penilaian medis ralan: %w", err)
	}

	rowsAffected, err := res.RowsAffected()
	if err != nil {
		return fmt.Errorf("gagal cek rows affected hapus penilaian medis ralan: %w", err)
	}
	if rowsAffected == 0 {
		return sql.ErrNoRows
	}
